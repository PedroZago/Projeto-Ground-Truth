...
void draw() {
  PImage img = loadImage("img1.png");
  PImage imgMask = loadImage("img1 - mask.png");
  PImage aux = loadImage("img1.png");
...
PImage thresholding(PImage aux2) {
  for (int y = 0; y < aux2.height; y++) {
    for (int x = 0; x < aux2.width; x++) {
      ...
      if (red(aux2.pixels[pos]) > 113 && x > 100 && x < 480 && y > 175 && y < 268 || blue(aux2.pixels[pos]) < 100 && x > 140 && x < 150 && y > 185 && y < 195) {
